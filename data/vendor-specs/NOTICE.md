...
| `xdr_openapi_reduced.json` | `github.com/tommynsong/cortex-mcp-custom-tools-openapi` (community transcription of the Cortex XDR reference) | **No licence** | **Key paths only** — of the reply, and of the request where it marks a member required; nothing from the repository is vendored (`scripts/cortex_openapi_spec.py`) |
