...
| `graph_beta_csdl_types.json` | `github.com/microsoftgraph/msgraph-metadata` (`clean_beta_metadata/cleanMetadata.xml`) | MIT | Type → property names and Edm types, for the closure over the types the mock serves from beta — today `tiIndicator`, which Microsoft retired from v1.0 and removed from its metadata with it (`scripts/graph_csdl_spec.py`) |
| `crowdstrike_gofalcon_reduced.json` | `github.com/CrowdStrike/gofalcon` (generated from the Falcon swagger) | MIT | Reduced to operation → 200-payload key paths, plus what each write route *accepts* — `request`, `request_paths`, `request_required` (`scripts/gofalcon_spec.py`) |
