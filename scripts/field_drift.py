...
KNOWN_EXCEPTIONS: dict[str, set[str]] = {
    ...
    # decommissionedAt is a real S1 API field present on agents but missing from the 2.1 spec
    "agents": {"activeDirectory.mail", "decommissionedAt"},
...
def wait_for_server(base_url: str, timeout: int = 30) -> bool:
    ...
    while time.time() < deadline:
        try:
            r = httpx.get(status_url, timeout=2)
