...
def _with_exposed_count(record: dict) -> dict:
    ...
    The record is keyed `vulnerabilityId` in the store and `id` on the wire, and the
    rename never happened: every entry of this collection answered `id: ""`,
    so nothing in it could be addressed. `GET /api/vulnerability/{id}` was
    ...
    return complete_mde(to_mde_resource(record, "vulnerabilityId"), "vulnerability")
...
def list_vulnerabilities(
    ...
    """
    records = [_with_exposed_count(record_dict(v)) for v in mde_vulnerability_repo.list_all()]
    if filter_str:
