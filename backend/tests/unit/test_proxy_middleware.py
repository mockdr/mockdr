...
To run against a real endpoint: replace the mock fixture with actual
credentials via environment variables — the test structure is identical.
...
def _make_scope(path: str = "/web/api/v2.1/threats", method: str = "GET",
                query: bytes = b"", scope_type: str = "http") -> dict:
    return {
        "type": scope_type,
        "method": method,
        "path": path,
...
def _fake_response(status: int = 200, body: str = '{"data":[]}',
...
async def test_dev_path_always_passes_through_in_record_mode() -> None:
    ...
    _state._config = ProxyConfig(mode="record", base_url="https://example.test", api_token="tok")
    mw = RecordingProxyMiddleware(inner_app)
...
async def test_record_mode_stores_recording() -> None:
    ...
    _state._config = ProxyConfig(
        mode="record",
        base_url="https://example.test/web/api/v2.1",
...
async def test_record_mode_forwards_response_to_client() -> None:
    ...
    # collected[0] = http.response.start, collected[1] = http.response.body
...
async def test_replay_mode_falls_through_when_no_match() -> None:
    ...
    mw = RecordingProxyMiddleware(inner_app)
    await mw(_make_scope(path="/web/api/v2.1/agents"), _make_receive(), AsyncMock())
    assert inner_called
...
async def test_record_and_forward_returns_true_on_success() -> None:
    ...
    with patch("httpx.AsyncClient", return_value=mock_client):
        result = await _record_and_forward(
            ...
            config_base_url="https://example.test/web/api/v2.1",
            config_token="fake-token",
            send=send,
        )
