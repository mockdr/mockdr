...
class TestProxyConfig:
    ...
    def test_empty_base_url_keeps_existing(self) -> None:
        proxy_commands.set_config(
            "record",
            base_url="https://original.example.com/web/api/v2.1",
        )
        proxy_commands.set_config("replay", base_url="")
        raw = proxy_queries.get_config_raw()
        assert raw.base_url == "https://original.example.com/web/api/v2.1"

    def test_base_url_trailing_slash_stripped(self) -> None:
        proxy_commands.set_config(
            "record",
            base_url="https://example.com/web/api/v2.1/",
        )
        assert proxy_queries.get_config_raw().base_url == "https://example.com/web/api/v2.1"

    def test_per_vendor_config(self) -> None:
        proxy_commands.set_config(
            "record",
            vendors=[
                {
        ...
        cfg = proxy_queries.get_config_raw()
        assert "crowdstrike" in cfg.vendors
        assert cfg.vendors["crowdstrike"].base_url == "https://api.crowdstrike.com"

    def test_invalid_vendor_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown vendor"):
            proxy_commands.set_config(
                "record",
                vendors=[{"vendor": "nope", "base_url": "http://x"}],
            )
...
class TestRecordings:
    ...
    def test_list_newest_first(self) -> None:
        ...
        r2 = ProxyRecording(
            **{**r1.__dict__, "id": "rec-2", "path": "/web/api/v2.1/agents"},
        )
    ...
    def test_find_recording_match(self) -> None:
        proxy_commands.add_recording(
            self._make_recording("GET", "/web/api/v2.1/threats"),
        )
    ...
    def test_find_recording_returns_most_recent(self) -> None:
        r1 = self._make_recording()
        r2 = ProxyRecording(
            **{**r1.__dict__, "id": "rec-2", "response_status": 201},
        )
        proxy_commands.add_recording(r1)
        proxy_commands.add_recording(r2)
        found = proxy_queries.find_recording("GET", "/web/api/v2.1/threats")
