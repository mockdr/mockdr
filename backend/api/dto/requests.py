...
class VendorAuthBody(BaseModel):
    """Auth config for a single vendor in the proxy."""
    type: str = "api_token"  # api_token | oauth2 | basic | hmac
    token: str = ""
    client_id: str = ""
    client_secret: str = ""
    token_url: str = ""
...
class VendorConfigBody(BaseModel):
    """Per-vendor proxy config."""
    vendor: str
    base_url: str = ""
    auth: VendorAuthBody = Field(default_factory=VendorAuthBody)
    enabled: bool = True
...
class ProxyConfigBody(BaseModel):
    ...
    mode: str = ""
    vendors: list[VendorConfigBody] = Field(default_factory=list)
