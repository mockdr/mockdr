...
describe('PlaybookView', () => {
  ...
  it('step X button triggers removeStep via DOM click', async () => {
    ...
    ;(w.vm as any).addStep()
    await w.vm.$nextTick()
    const removeBtn = w.find('button.p-1.shrink-0')
  ...
  it('step action select triggers onStepActionChange via DOM change', async () => {
    ...
    ;(w.vm as any).addStep()
    await w.vm.$nextTick()
    const select = w.find('select.bg-s1-bg.border.border-s1-border.rounded-sm.px-2')
  ...
  it('alert step mitreTechnique input updates via DOM setValue', async () => {
    ...
    const actionSelect = w.find('select.bg-s1-bg.border.border-s1-border.rounded-sm.px-2')
