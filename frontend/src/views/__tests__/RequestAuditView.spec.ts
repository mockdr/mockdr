...
import RequestAuditView from '@/views/RequestAuditView.vue'
...
describe('RequestAuditView', () => {
  ...
  it('Refresh button triggers fetchLogs via DOM click', async () => {
    const { systemApi } = await import('../../api/system')
    const w = mount(RequestAuditView, { global: { stubs: { EmptyState: true, LoadingSkeleton: true } } })
    await flushPromises()
    vi.mocked(systemApi.listRequests).mockClear()
    const refreshBtn = w.findAll('button').find(b => b.text().includes('Refresh'))
    await refreshBtn!.trigger('click')
    await flushPromises()
    expect(systemApi.listRequests).toHaveBeenCalled()
  })

  it('Clear button triggers clearLogs via DOM click', async () => {
    const { systemApi } = await import('../../api/system')
    const w = mount(RequestAuditView, { global: { stubs: { EmptyState: true, LoadingSkeleton: true } } })
    await flushPromises()
    ...
    if (clearBtn) {
      ...
      await flushPromises()
      expect(systemApi.clearRequests).toHaveBeenCalled()
    }
  })
})
